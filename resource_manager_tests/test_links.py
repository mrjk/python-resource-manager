...
from resource_manager.exceptions import ResourceConfigError, ResourceTypeError, ResourceLinkError
...
class TestResourceLink:
    ...
    def test_link_initialization_with_dict(self):
        ...
        link = ResourceLink({"kind": "test.kind", "instance": "instance1"})
        
        assert link.kind == "test.kind"
    ...
    def test_link_initialization_with_string(self):
        ...
        link = ResourceLink("test.kind.instance1")
        
        assert link.kind == "test.kind"
        assert link.instance == "instance1"
        assert link.mod is None
        assert link.raw_value == "test.kind.instance1"
    ...
    def test_link_initialization_invalid_format(self):
        ...
        with pytest.raises(ResourceConfigError):
            ResourceLink("test.kind.instance1.extra")
            
    def test_rule_property(self):
        """Test the rule property returns the correct format."""
        link1 = ResourceLink({"kind": "test.kind", "instance": "instance1"})
        assert link1.rule == "test.kind.instance1"
        
        link2 = ResourceLink({"kind": "test.kind"})
        assert link2.rule == "test.kind.*"
        
    def test_resource_property(self):
        """Test the resource property returns the parent resource."""
        resource = Resource("test_resource")
        link = ResourceLink({"kind": "test.kind"}, parent=resource)
