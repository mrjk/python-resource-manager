...
class TestResolverExceptions:
    ...
    def test_resolver_unresolvable_requirements_error(self):
        ...
        manager.add_resource("app", config={"requires": ["database.main"]})

        # Create resolver requesting the app
        resolver = DepBuilder(resources=manager, feature_names=["app"])
    ...
    def test_resolver_missing_feature_error(self):
        ...
        # Add a resource
        manager.add_resource("app", config={"provides": ["app.feature"]})

        # Create resolver requesting a non-existent feature
        resolver = DepBuilder(resources=manager, feature_names=["missing.feature"])
