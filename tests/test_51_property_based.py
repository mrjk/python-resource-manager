...
@st.composite
def resource_configs(draw, min_provides=0, max_provides=5, min_requires=0, max_requires=5):
    ...
    provides = draw(st.lists(capability_names(), min_size=num_provides, max_size=num_provides, unique=True))
    requires = draw(st.lists(capability_names(), min_size=num_requires, max_size=num_requires, unique=True))
...
def resource_collections(draw, min_resources=1, max_resources=10):
    ...
    names = draw(st.lists(resource_names(), min_size=num_resources, max_size=num_resources, unique=True))
...
def resolvable_resource_collections(draw):
    ...
    for i in range(num_extra):
        ...
        num_requires = draw(st.integers(min_value=0, max_value=min(3, len(all_capabilities))))
        requires = draw(st.sampled_from(all_capabilities) if all_capabilities else st.just([]))
        ...
        for i, res_name in enumerate(resolver.dep_order):
            ...
            for req in resource.requires:
                ...
                for provider_name in providers:
                    ...
                    assert provider_idx < i, f"Provider {provider_name} should come before {res_name}"
